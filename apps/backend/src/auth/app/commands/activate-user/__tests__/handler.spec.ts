...
import { ActivateUserHandler } from '../handler';
import { ActivateUserCommand } from '../command';
...
describe('ActivateUserHandler', () => {
  ...
  it('should activate user successfully', async () => {
    ...
    user.deactivate(); // Deactivate first so we can activate
  ...
  it('should throw UserAlreadyActiveException when user is already active', async () => {
    // Arrange
    const userId = UUID.generate().getValue();
    const command = new ActivateUserCommand(userId);
    ...
    // User is active by default after registration

    userFactory.loadById.mockResolvedValue(user);

    // Act & Assert
    await expect(handler.execute(command)).rejects.toThrow(UserAlreadyActiveException);
    expect(userWriteRepository.save).not.toHaveBeenCalled();
  });

  it('should activate user with multiple roles', async () => {
    // Arrange
    const userId = UUID.generate().getValue();
    const command = new ActivateUserCommand(userId);
  ...
  it('should activate user and preserve email verification status', async () => {
    // Arrange
    const userId = UUID.generate().getValue();
    const command = new ActivateUserCommand(userId);
