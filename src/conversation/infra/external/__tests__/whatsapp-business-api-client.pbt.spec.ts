...
describe('WhatsAppBusinessApiClient - Property Tests', () => {
  ...
  it(
    'should retry exactly 3 times on failure before throwing error',
    async () => {
    ...
    10000,
  ); // 10 second timeout for retry delays
  ...
  it(
    'should succeed on retry if first attempts fail',
    async () => {
      const phoneNumber = '+1234567890';
      const message = 'Test message';
  ...
  it(
    'should retry for sendInteractiveButtons with same logic',
    async () => {
      const phoneNumber = '+1234567890';
      const message = 'Test message';
      ...
      // Act & Assert
      await expect(
        client.sendInteractiveButtons(phoneNumber, message, buttons),
      ).rejects.toThrow('Failed to send WhatsApp interactive buttons after 3 attempts');
  ...
  it(
    'should retry for sendLocation with same logic',
    async () => {
      const phoneNumber = '+1234567890';
      const location = {
        latitude: 40.7128,
