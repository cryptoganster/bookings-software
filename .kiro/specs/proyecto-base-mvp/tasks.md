...
- [x] 2. Setup proyecto NestJS y configuración inicial
  ...
  - [x] 2.6 Configurar TypeScript
    ...
    - Configurar paths para imports (@shared/_, @booking/_, etc.)
  ...
  - [x] 2.8 Crear estructura de carpetas
    ...
    - Crear src/conversation/ con estructura similar
...
- [x] 5. Implementar Booking Domain Layer
  ...
  - [x] 4.5 Escribir tests unitarios para Appointment Aggregate
    - Crear src/booking/domain/aggregates/**tests**/appointment.spec.ts
...
- [x] 13. Implementar flujo conversacional básico
  ...
  - [x] 12.1 Crear Conversation Aggregate (simplificado)
    ...
    - _Requirements: 7.1_
  - [x] 12.2 Implementar ProcessIncomingMessageHandler
  ...
  - [x] 12.3 Implementar lógica de disponibilidad
    ...
    - _Requirements: 8.1_
  - [x] 12.4 Integrar creación de cita en flujo
  ...
  - [x] 13.5 Escribir tests E2E para flujo conversacional
...
- [x] 14. Implementar Availability Bounded Context (Migración Agresiva Guiada por Pruebas)
  ...
  - [x] 14.14 Post-migración: Verificar tests completos
    ...
    - _Requirements: 1.1, 8.1_
  - [x] 14.15 Limpiar archivos duplicados
    ...
    - _Requirements: 8.1_
  - [x] 14.16 Crear comandos básicos de Availability
...
- [-] 15. Implementar Autenticación JWT
  ...
  - [x] 13.5 Escribir property test para JWT
    ...
    - _Requirements: 9.3_
  - [x] 14.6 Escribir property test para guards
...
- [x] 16. Implementar REST API para Panel Web
  - Crear rama: `git checkout -b feature/rest-api`
  - [x] 14.1 Crear DTOs con validación
    ...
    - _Requirements: 10.4_
  - [x] 14.2 Crear AppointmentController
...
- [x] 19. Configurar AppModule principal
...
- [ ] 22. Implementar endpoint de estadísticas para Dashboard
  - Crear rama: `git checkout -b feature/appointment-stats-endpoint`
  - [ ] 22.1 Crear GetAppointmentStatsQuery
    - Crear `src/booking/app/queries/get-appointment-stats/query.ts`
    - Definir `GetAppointmentStatsQuery extends Query<AppointmentStatsReadModel>`
    - Incluir parámetro businessId
    - _Requirements: Frontend Dashboard (Task 31)_
  - [ ] 22.2 Crear AppointmentStatsReadModel
    - Crear `src/booking/domain/read-models/appointment-stats.ts`
    - Definir interface con: appointmentsToday, appointmentsThisWeek
    - _Requirements: Frontend Dashboard (Task 31)_
  - [ ] 22.3 Implementar GetAppointmentStatsHandler
    - Crear `src/booking/app/queries/get-appointment-stats/handler.ts`
    - Decorar con `@QueryHandler(GetAppointmentStatsQuery)`
    - Inyectar `IAppointmentReadRepository`
    - Calcular appointmentsToday: contar citas con fecha = hoy
    - Calcular appointmentsThisWeek: contar citas con fecha en semana actual
    - Usar date-fns para cálculos de fechas (startOfDay, startOfWeek, endOfWeek)
    - Retornar `AppointmentStatsReadModel`
    - _Requirements: Frontend Dashboard (Task 31)_
  - [ ] 22.4 Agregar métodos al IAppointmentReadRepository
    - Agregar `countByDateRange(businessId: string, startDate: Date, endDate: Date): Promise<number>`
    - Implementar en `AppointmentReadRepository` usando QueryBuilder
    - Escribir tests unitarios para el método
    - _Requirements: Frontend Dashboard (Task 31)_
  - [ ] 22.5 Agregar endpoint en AppointmentController
    - Agregar `@Get('stats')` en `AppointmentController`
    - Inyectar `QueryBus`
    - Ejecutar `GetAppointmentStatsQuery` con businessId del usuario
    - Retornar resultado
    - Proteger con `@UseGuards(JwtAuthGuard)`
    - _Requirements: Frontend Dashboard (Task 31)_
  - [ ] 22.6 Registrar handler en BookingModule
    - Agregar `GetAppointmentStatsHandler` a providers de `BookingModule`
    - _Requirements: Frontend Dashboard (Task 31)_
  - [ ] 22.7 Escribir tests para GetAppointmentStatsHandler
    - Test: retorna 0 cuando no hay citas
    - Test: cuenta correctamente citas de hoy
    - Test: cuenta correctamente citas de la semana
    - Test: no cuenta citas canceladas
    - Test: respeta filtro por businessId
    - _Requirements: Frontend Dashboard (Task 31)_
  - [ ] 22.8 Escribir test E2E para endpoint
    - Test: `GET /api/appointments/stats` retorna estadísticas correctas
    - Test: requiere autenticación (401 sin token)
    - Test: retorna solo stats del business del usuario autenticado
    - _Requirements: Frontend Dashboard (Task 31)_
  - [ ] 22.9 Actualizar frontend para usar endpoint real
    - Modificar `apps/frontend/src/widgets/StatsCards/model/useStats.ts`
    - Descomentar llamada real a API: `apiClient.get<StatsData>("/appointments/stats")`
    - Eliminar mock data y simulación de delay
    - Verificar que WebSocket invalida query correctamente en eventos
    - _Requirements: Frontend Dashboard (Task 31)_
  - [ ] 22.10 Verificar integración completa
    - Iniciar backend y frontend
    - Login en frontend
    - Verificar que Dashboard muestra estadísticas reales
    - Crear una cita y verificar que stats se actualizan (vía WebSocket)
    - Cancelar una cita y verificar que stats se actualizan
    - _Requirements: Frontend Dashboard (Task 31)_
  - Commit: `git commit -m "feat: implement appointment stats endpoint for dashboard"`
  - Push y crear PR a develop
  - _Requirements: Frontend Dashboard (Task 31)_

- [ ] 23. Release a producción
