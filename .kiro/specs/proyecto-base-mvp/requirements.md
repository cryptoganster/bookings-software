...
1. WHEN se crea un WriteRepository THEN el Sistema SHALL implementar solo métodos de escritura (save, delete) usando el modelo de escritura
2. WHEN se necesita cargar un Aggregate para modificación THEN el Sistema SHALL usar un Factory que implemente loadById() y retorne el Aggregate con su versión
3. WHEN se crea un ReadRepository THEN el Sistema SHALL implementar queries optimizadas usando el modelo de lectura
4. WHEN se guarda un Aggregate THEN el Sistema SHALL usar UPDATE con WHERE version para implementar Optimistic Locking
5. WHEN falla una actualización por versión THEN el Sistema SHALL detectar affected=0 y lanzar ConcurrencyException
6. WHEN se mapea entre dominio y persistencia THEN el Sistema SHALL usar Mappers dedicados para WriteModel y ReadModel

**NOTA:** Este requirement fue actualizado para reflejar el Factory Pattern que mantiene CQRS estricto. Ver `.kiro/steering/factory-pattern.md` para detalles.
